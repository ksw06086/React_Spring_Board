...
export default function Authentication() {
    ...
    const SignInCard = () => {
        ...
        const signInResponse = (responseBody: SignInResponseDto | ResponseDto | null) => {
            if (!responseBody) {        // 백엔드 서버가 안켜졌거나 도메인이 잘못되었거나 할 때 null 반환 됨
                alert('네트워크 이상입니다.');
                return;
            }
    ...
    const SignUpCard = () => {
        ...
        // event handler: 핸드폰 번호 인풋 키 다운 이벤트 처리   //
            ...
            <div className='auth-card'>
                <div className='auth-card-box'>
                    <div className='auth-card-top'>
                        ...
                        {page === 1 && (
                        <>
                            <InputBox ref={emailRef} label='이메일 주소*' type='text' placeholder='이메일 주소를 입력해주세요.' error={isEmailError} value={email} onChange={onEmailChangeHandler} message={emailErrorMessage} onKeyDown={onEmailKeyDownHandler} />
                            <InputBox ref={passwordRef} label='패스워드*' type={passwordType} placeholder='비밀번호를 입력해주세요.' error={isPasswordError} value={password} onChange={onPasswordChangeHandler} message={passwordErrorMessage} icon={passwordButtonIcon} onButtonClick={onPasswordButtonClickHandler} onKeyDown={onPasswordKeyDownHandler} />
                            <InputBox ref={passwordCheckRef} label='패스워드 확인*' type={passwordCheckType} placeholder='비밀번호를 다시 입력해주세요.' error={isPasswordCheckError} value={passwordCheck} onChange={onPasswordCheckChangeHandler} message={passwordCheckErrorMessage} icon={passwordCheckButtonIcon} onButtonClick={onPasswordCheckButtonClickHandler} onKeyDown={onPasswordCheckKeyDownHandler} />
                        </>
                        )}
                        {page === 2 && (
                        <>
                            <InputBox ref={nicknameRef} label='닉네임*' type='text' placeholder='닉네임을 입력해주세요.' error={isNicknameError} value={nickname} onChange={onNicknameChangeHandler} message={nicknameErrorMessage} onKeyDown={onNicknameKeyDownHandler} />
                            <InputBox ref={telNumberRef} label='핸드폰 번호*' type='text' placeholder='핸드폰 번호를 입력해주세요.' error={isTelNumberError} value={telNumber} onChange={onTelNumberChangeHandler} message={telNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler} />
                            <InputBox ref={addressRef} label='주소*' type='text' placeholder='우편번호 찾기' error={isAddressError} value={address} onChange={onAddressChangeHandler} message={addressErrorMessage} icon='expand-right-light-icon' onButtonClick={onAddressButtonClickHandler} onKeyDown={onAddressKeyDownHandler} />
